...
// Extensions eligible for auto-push when detected in MiMoCode's response
...
async function sendToMimoCode(
  ...
  try {
    ...
    if (result.error && queryOptions.resume) {
      ...
      sessionStore.save(account.accountId, session);
      const retryResult = await mimocodeQuery(queryOptions);
    ...
    // Auto-push deliverable files mentioned in MiMoCode's response
    if (result.text) {
